...
- **PID + 前馈计算**: `100 Hz`
- **串口反馈上报**: `100 ms`
- **运动命令通信看门狗**: `200 ms` 未收到有效运动命令时自动清零目标与 PWM
...
切换帧只会在协议解析器处于空闲边界时启动识别。已经进入 MOWEN 速度帧后，载荷中的 `0xFE`（例如负数 `int16` 的高字节）会作为普通速度数据保留，不会再被误判为切换帧起始字节。

正式速度帧和 ASCII `VEL` / `PWMTEST` 等运动指令需要周期刷新。当前通信看门狗超时为 `200 ms`，上位机建议每 `20~100 ms` 发送一次；停止刷新后固件会自动切换到 `STOP` 并清零全部 PWM。
